...
export default function MessagingPage() {
    ...
    return (
        <div className="flex h-[calc(100vh-120px)] bg-white rounded-[32px] overflow-hidden shadow-xl border border-gray-100">
            {/* Sidebar */}
            <div className={`w-full md:w-80 border-r border-gray-50 flex flex-col ${selectedUser ? 'hidden md:flex' : 'flex'}`}>
                <div className="p-6 border-b border-gray-50">
                    <h3 className="text-xl font-extrabold text-gray-900 mb-4">Mesajlar</h3>
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 h-4 w-4" />
                        <input
                            type="text"
                            placeholder="İsim veya rol ile ara..."
                            className="w-full bg-gray-50 border-none rounded-2xl py-2.5 pl-10 pr-4 text-sm focus:ring-4 focus:ring-emerald-500/5 transition-all outline-none"
                            value={searchQuery}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    {filteredConversations.map(conv => (
                        <div
                            ...
                            className={`p-4 flex items-center gap-4 cursor-pointer transition-all hover:bg-gray-50 border-l-4 ${selectedUser?.id === conv.id ? 'border-emerald-500 bg-emerald-50/30' : 'border-transparent'}`}
                        >
                            <div className="relative">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500 truncate">{conv.lastMessage || conv.role}</p>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {/* Chat Window */}
            <div className={`flex-1 flex flex-col bg-gray-50/50 ${!selectedUser ? 'hidden md:flex items-center justify-center' : 'flex'}`}>
                {selectedUser ? (
                    <>
                        {/* Header */}
                        <div className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-8 shrink-0">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="font-bold text-gray-900 leading-tight">{selectedUser.name}</p>
                                ...
                                return (
                                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[70%] group ${isMe ? 'items-end' : 'items-start'}`}>
                                            <div className={`p-4 rounded-[20px] ${isMe ? 'bg-emerald-600 text-white rounded-tr-none shadow-lg shadow-emerald-200/50' : 'bg-white text-gray-800 rounded-tl-none border border-gray-100'}`}>
                        ...
                        {/* Input */}
                        <div className="p-6 bg-white border-t border-gray-100 border-b rounded-b-[32px]">
                            <form onSubmit={handleSendMessage} className="flex gap-4">
                                <input
                                    type="text"
                                    placeholder="Bir mesaj yazın..."
                                    className="flex-1 bg-gray-50 border-none rounded-2xl px-6 py-3.5 text-sm focus:ring-4 focus:ring-emerald-500/5 transition-all outline-none"
                                    value={newMessage}
                                ...
                                <button
                                    ...
                                    className="h-12 w-12 bg-emerald-600 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-200 transition-all hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:translate-y-0"
                                >
                    ...
                    <div className="text-center p-8">
                        ...
                        <h4 className="text-xl font-bold text-gray-900 mb-2">Hemen İletişime Geçin</h4>
