...
import { Plus, Check, Trash2, Clock, Calendar, User, Users } from 'lucide-react';
                ...
                <div className="lg:col-span-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {loading ? (
                            ...
                            return (
                                <div key={task.id} className={`bg-card p-6 rounded-[32px] border transition-all group relative overflow-hidden ${task.isCompleted ? 'opacity-40 grayscale-[0.5] border-border' : 'border-border shadow-sm hover:shadow-md'}`}>
                                    {/* Top row */}
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex items-center gap-3">
                                            <button
                                                onClick={() => toggleComplete(task)}
                                                className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all ${task.isCompleted ? 'bg-muted text-muted-foreground/40 border border-border' : 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20'}`}
                                            >
                                                {task.isCompleted ? <Check size={20} /> : <div className="w-5 h-5 rounded-full border-2 border-white" />}
                                            </button>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-muted border border-border">
                                            <Clock size={12} className={status === 'overdue' && !task.isCompleted ? 'text-red-500' : task.isCompleted ? 'text-muted-foreground/40' : 'text-emerald-600'} />
                                            <span className={`text-[11px] font-bold ${status === 'overdue' && !task.isCompleted ? 'text-red-600' : task.isCompleted ? 'text-muted-foreground/40' : 'text-muted-foreground'}`}>
                                        ...
                                        {task.customer && (
                                            <button
                                                ...
                                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 text-emerald-600 border border-emerald-100 hover:bg-emerald-100 transition-all shadow-sm"
                                            >
                                                <Users size={12} />
                                                <span className="text-[10px] font-bold uppercase tracking-widest">{task.customer.firstName} {task.customer.lastName}</span>
                                            </button>
                                        )}
                                    </div>
                                    ...
                                    {/* Corner Decoration */}
                                    <div className={`absolute top-0 right-0 w-16 h-1 bg-gradient-to-l ${status === 'overdue' && !task.isCompleted ? 'from-red-500' : task.isCompleted ? 'from-muted' : 'from-emerald-500'}`}></div>
                                </div>
                            );
                        })}
                    </div>
