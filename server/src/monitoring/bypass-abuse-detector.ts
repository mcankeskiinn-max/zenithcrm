export type BypassPermission = 'SUPER_ADMIN' | 'SYSTEM_JOB' | 'DATA_ANALYST' | 'ADMIN';
...
const BYPASS_TIME_WINDOW_MINUTES = 60;
const MAX_BYPASS_ATTEMPTS = 10;
const OFF_HOURS_START = 19;
const OFF_HOURS_END = 8;
const MAX_SAME_REASON = 50;
...
export class BypassAbuseDetector {
    ...
    async checkAbuse(userId: string, reason?: string): Promise<BypassAlert | null> {
        ...
        if (reason) {
            ...
            if (reasonCount > MAX_SAME_REASON) {
                return {
...
export const bypassAbuseDetector = new BypassAbuseDetector();
